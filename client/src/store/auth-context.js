...
const AuthContext = React.createContext({
  token: "",
  isLoggedIn: false,
  login: (token, expirationTime, name, userId, email) => {},
  logout: () => {},
});
...
export const AuthContextProvider = (props) => {
  ...
  const loginHandler = (token, expirationTime, name, userId, email) => {
    setToken(token);
    localStorage.setItem(
      "user",
      JSON.stringify({ token, expirationTime, name, userId, email })
    );
  };
